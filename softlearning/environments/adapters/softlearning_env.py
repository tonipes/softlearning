    ...
    def get_path_infos(self, paths, *args, **kwargs):
        ...
        for path in paths:
            ...
            for info_key, info_values in path_results.items():
                if np.array(info_values).dtype.kind not in "UO":
                    # NOTE: this crashes if info_values has objects or strings
                    # The above check was added because ml-agents gives some string/object outputs
